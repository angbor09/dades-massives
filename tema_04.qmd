---
title: "Mapes i visualització espacial"
author: ""
date: today
...
# Mapes de punts
Els mapes que representen dades mitjançant punts es coneixen com mapes de punts. En aquest tipus de mapes, cada punt se situa en la seva ubicació geogràfica exacta (per exemple, mitjançant coordenades de latitud i longitud) i pot representar un únic esdeveniment o observació (com la ubicació d'una ciutat, un arbre o una botiga).
...
# Mapes de coropletes
Un mapa de coropletes és un tipus de mapa temàtic en el qual les diferents àrees geogràfiques (països, comunitats autònomes, municipis, etc.) es pinten amb colors o intensitats de color diferents en funció del valor d'una variable. Com més fosc o intens és el color d'una regió, més elevat sol ser el valor de la variable representada en aquella zona. Aquest tipus de mapa és molt útil per identificar patrons espacials, com concentracions regionals o gradients geogràfics, d'un cop d'ull. Un exemple habitual és un mapa que representi la renda mitjana per comunitat autònoma o la taxa d'atur per província, on cada regió es tenyeix segons el seu valor respecte a la resta.
...
A continuació, creem el mapa. Fixeu-vos que, a diferència dels `geom_point()` anteriors, `geom_sf()` no requereix especificar `x` i `y` perquè la geometria ja està continguda a la columna espacial del dataframe (`geometry`).
...
Com hem explicat, un mapa de coropletes és un mapa temàtic que utilitza un color per representar una variable quantitativa, que en aquest cas serà la població de cada comunitat autònoma.
...
Ara ja podem crear el mapa. En el següent codi, la funció `scale_fill_gradient()` defineix una escala de color contínua entre dos extrems: el color per als valors més baixos (argument `low`) i el color per als valors més alts (argument `high`). R interpola automàticament tots els tons intermedis entre aquests dos colors en funció del valor de cada observació, de manera que les diferències de color reflecteixen les diferències numèriques de la variable. A més d'aquests dos arguments, també es pot personalitzar el format dels números que s'hi mostren (`labels`), com s'ha fet en l'exemple amb `label_comma()` per mostrar la població amb el punt com a separador de milers.
...
# Mapa de punts dels Estats Units
...
US_cities <- data.frame(
  ...
  longitud = c(-95.3802, -94.9147, -74.0060, -122.4194, -122.3321))
